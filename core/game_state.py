...
from config import INITIAL_SECURITY_SCORE
from core.scoring import clamp, SCORE_MAX
...
def init_game_state() -> None:
    ...
    if "max_unlocked_scene" not in st.session_state:
        st.session_state.max_unlocked_scene = 0
    if "completed_scenes" not in st.session_state:
        st.session_state.completed_scenes = {}
